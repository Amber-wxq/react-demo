...
function Item(props:ItemProp):JSX.Element{
    ...
    const [desc,setDesc]=useState(itemValue);
    const [deadline,setDeadline]= useState(itemDeadline)

   
    return (
        <li className="item">
             <label >完成 <input type="checkbox" name="" value=""/></label>
    
            <span className="item-value">do:{desc}</span>
            <span className="item-deadline" >截止日期：{deadline}</span>
            <br />
            <label >
